    ...
    <div className='w-full h-[100vh] bg-slate-300 flex justify-center items-center'>
      ...
      <div className="w-full max-w-[500px] h-[600px] bg-white rounded-lg shadow-gray-300 shadow-lg flex flex-col gap-[30px]">
        ...
        <div className='w-full h-[200px] bg-[#FFE500] rounded-b-[30%] shadow-gray-200 shadow-md flex items-center justify-center'>
          <h1 className='text-gray-600 font-bold text-[30px]'>Welcome to <span className='text-white'>Yo-chat!</span></h1>
        </div>

        <form className='w-full flex justify-center items-center gap-4 flex-col mt-1' onSubmit={handleSignUp}>
          <input type="text" placeholder='username' className='w-[90%] h-[55px] outline-none border-2 border-[#FFE500] px-[20px] py-[10px] bg-white rounded-lg shadow-gray-200 shadow-lg text-gray-700 text-[19px]' onChange={(e)=>setusername(e.target.value)} value={username}/>
...
{Err && <p className='text-red-600' >{Err}</p>}
           ...
           <button className='bg-[#FFE500] px-[20px] py-[10px] rounded-2xl shadow-gray-200 shadow-lg mt-3 w-[200px]'>sign Up</button>
           <p className='mt-2 text'>Already Have a Account ? <span className='text-[#ffe600] text-[bold] cursor-pointer hover:underline' onClick={()=>navigate("/login")}>login</span> </p>
        </form>
